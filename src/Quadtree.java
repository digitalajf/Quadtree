...
public class Quadtree {
    ...
    * "build_Quadtree" to begin the process.&nbsp;If square is "true", before building the
    * Quadnodes, size the root node so that it is square (both width and height will be the largest
    * of the window's width and height).&nbsp;If the root Quadnode is not square, none of the
    * Quadnodes will be square and ghost collisions may occur more frequently on the axis where
    * Quadnodes are wider (or taller).&nbsp;A square quadtree in a rectangular screen or window
    * means part of the Quadtree is off screen but performance should remain the same.
    *
    ...
    * @param square If set to "true" quadtree will have a square shape otherwise the quadtree will
    * be rectangular.
    *****************************************************************************
    */
   ...
   /**
    * *****************************************************************************
    * Recursively create and return the Quadnodes until the Quadtree reaches the maximum allowed
    * tree depth defined by "max_Tree_Depth".&nbsp;The Quadtree will be completed with empty AABB
    * lists and ready to be used for fast recursive compartmentalization of AABBs provided to the
    * Quadtree.
    *
   ...
   /**
    * **************************************************************************
    * Iterate through the given Quadnodes's AABB list and check to see where the AABBs lie in the
    * given Quadnode's four sub-quadnodes and add it to that sub-quadnode.&nbsp;After having passed
    * through the AABB list, recursively call this method against this quadnodes's sub-quadnodes
    * that contain at least 2 items within its AABB list and continue this process until either the
    * maximum tree depth has been reached or recursive calls do not present a situation which
    * requires further searching.&nbsp;If the maximum tree depth is reached, all AABBs within the
    * quadnode that reached the maximum tree depth are considered to be within close proximity to
    * each other.
    *
   ...
   /**
    * *****************************************************************************
    * Recursively iterate down the Quadtree and clear all AABB lists within the Quadnodes.
    *
    ...
    *
    * @param wdth The required width of this quadtree. If the value of "square" is "true", wdth will
    * be adjusted so that its value is that of the largest of wdth and hght.
    * @param hght The required height of this quadtree. If the value of "square" is "true", hght
    * will be adjusted so that its value is that of the largest of wdth and hght.
    *****************************************************************************
    */
   ...
   /**
    * *****************************************************************************
    * Set the maximum tree depth to "mtd".&nbsp;The quadtree must be rebuilt if the tree depth is
    * changed.
    *
    * @param mtd The maximum tree depth for this quadtree.
    *****************************************************************************
    ...
    *
    * @param target The AABB which will be set as the "nearby" AABB for all items in aabb_List.
    * @param aabb_List The AABB list who's contents is to have all their "nearby" AABBs set to
    * "target".
    *****************************************************************************
    */
   ...
   /**
    * *****************************************************************************
    * Update the Quadtree by emptying the leaf Quadnodes' AABB lists and then inserting the AABBs
    * one after another into the Quadtree.&nbsp;All AABB "nearby_AABBs" lists cleared until it is
    * determined again that there are AABBs nearby as determined by the "insert" method.
    ******************************************************************************
    */
   public void update() {
